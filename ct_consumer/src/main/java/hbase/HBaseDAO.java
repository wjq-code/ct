...
public class HBaseDAO {
    ...
    public void put(String ori) {
        try {
            ...
            String callee = splitOri[2];
            String builderTime = splitOri[4];
            String duration = splitOri[5];

            String regionCode = HBaseUtil.genRegionCode(caller, builderTime, regions);

            String buildTimeReplace = sdf2.format(sdf1.parse(builderTime));
            String buildTimeTs = String.valueOf(sdf1.parse(builderTime).getTime());
            ...
            put.addColumn(Bytes.toBytes("f1"), Bytes.toBytes("build_time"), Bytes.toBytes(builderTime));
